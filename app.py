...
cloud_model_location = "1sDToEFspnMeSYxapsv7swZltaZVRRCS2"
...
def load_model():
    ...
    f_checkpoint = Path("model/model_scratch.pt")
    ...
    disease_classifier.load_state_dict(torch.load(f_checkpoint, map_location=torch.device('cpu')))
...
#     disease_classifier = DiseaseClassifier()
#     disease_classifier.load_state_dict(torch.load("/content/model_scratch.pt", map_location=torch.device('cpu')))
#     return disease_classifier
...
def run_app(img_path):
    ...
    name, ids = predict_image_class(load_model(), class_names, img_path) 
    
    return ids  

# def plot_graph(model):
#     sm = nn.Softmax(dim=1)
#     probabilities = sm(model)

#     per = probabilities.detach().numpy() * 100
#     per = per[0,:]
#     df = pd.DataFrame(data=per, index=class_names)
#     ax = df.plot(kind="bar", figsize=(20,7))
#     for p in ax.patches:
...
def main():
    ...
    if choice == 'Home':
        ...
        if uploaded_file is not None:
            ...
            st.image(image, width=150, height=150)
